...
estMedian:
	...
	li	$t3, 0
	lw	$t3, ($t0)
	addu	$t2, $t2, $t3		# estMed set to $t2

	move	$t3, $t1		# length set to $s2
	remu 	$t3, $t1, 2		# check remainder if odd or even
	bnez	$t3, oddOption

evenOption:
	divu	$t3, $t1, 2		# divide length by 2

evenLoop:
	subu	$t3, $t3, 1		# get to the position in array
	addu	$t0, $t0, 4
	bnez 	$t3, evenLoop
...
trapAreas:
	...
	move	$t3, $a3			# set length
	lw	$t4, ($fp)			# get addr of tAreas
	li	$t5, 0
...
areasLoop:
	...
	addu	$t5, $t0, $t1			# $t5 = aSides[i] + cSides[i]
	divu	$t5, $t5, 2			# $t5 = $t5 / 2
	mul	$t5, $t5, $t2			# $t5 = $t5 * heights[i]

	sw	$t5, ($t4)			# store $t5 into tAreas array
...
trapStats:
	...
	addu	$fp, $sp, 4
	lw	$t0, ($fp)		# set addr of min to $t0
	addu	$fp, $fp, 4
	lw	$t1, ($fp)		# set addr of med to $t1
	addu	$fp, $fp, 4
	lw	$t2, ($fp)		# set addr of max to $t2
	move	$t3, $a1		# set len to $t4	

	lw	$t4, ($a0)		# set addr of list to $t4
	lw	$t5, ($a2)		# set addr of fSum to $t5
	lw	$t6, ($a3)		# set addr of fAve to $t6
